...
import Link from 'next/link'; // Import Link for interactive navigation
import React from 'react';

// Assuming you have a custom hook or context for system status
// If you want real status, you would use:
// import { useDashboard } from '../../StoreProvider'; 

const Sidebar: React.FC = () => {
  // const { isConnected } = useDashboard(); // Example of pulling real status
  const systemStatus = 'OPERATIONAL'; 
  const statusColor = 'text-green-400';
  const statusDotColor = 'bg-green-400';

  // --- Cyber Threat Scenarios Data (Now Clickable) ---
  const threatScenarios = [
    { name: 'DDoS Attack', icon: '🚨', description: 'Intense denial of service against the network', href: '/attack/ddos' },
    { name: 'Zero-Day Exploit', icon: '🦠', description: 'New, unpatched vulnerability detected', href: '/attack/zeroday' },
    { name: 'Ransomware Alert', icon: '🔒', description: 'System files may be compromised', href: '/attack/ransomware' },
    { name: 'Network Probe', icon: '📡', description: 'Routine scanning and reconnaissance', href: '/attack/probe' },
  ];

  return (
    <div className="w-64 flex flex-col bg-gray-800 text-white p-6 h-full shadow-2xl">
      ...
      <div className="bg-gray-700 p-4 rounded-lg mb-8">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className={`text-base font-semibold ${statusColor}`}>{systemStatus}</span>
        </div>
      </div>

      {/* 3. Cyber Threat Scenarios (Clickable links) */}
      <div className="mb-8">
        ...
        {threatScenarios.map((scenario) => (
          <Link 
            key={scenario.name} 
            href={scenario.href} // Uses next/link for client-side navigation
            className="flex flex-col p-3 rounded-lg hover:bg-gray-700 transition duration-150 cursor-pointer mb-2"
          >
            <div className="flex items-center space-x-2">
      ...
      <div className="mt-auto pt-4 border-t border-gray-700">
        <Link href="/settings" className="flex items-center text-sm text-gray-400 hover:text-cyan-400">
          ⚙️ Settings
        </Link>
      </div>
    </div>
