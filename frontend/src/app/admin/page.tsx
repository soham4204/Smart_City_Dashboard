...
import { 
  ShieldCheck, Save, Trash2, PlusCircle, Settings, AlertTriangle, ArrowLeft, Lock, Key, MapPin 
} from 'lucide-react';

// --- Dynamic Import for Map (Disable SSR) ---
const AdminMap = dynamic(() => import('@/components/admin/AdminMap'), { 
  ssr: false,
  loading: () => <div className="h-96 w-full bg-gray-800 animate-pulse rounded-xl flex items-center justify-center text-gray-500">Loading Interactive Map...</div>
});
        ...
        <div className="p-6 max-w-7xl mx-auto w-full flex-grow">
          ...
          <header className="flex justify-between items-center mb-8 border-b border-gray-800 pb-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setIsAuthenticated(false)} className="px-4 py-2 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded-lg border border-red-500/30 text-sm font-medium transition-all">Secure Logout</button>
            </div>
          </header>
          ...
          <div className="mb-8">
             <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-lg font-semibold text-gray-200">Infrastructure Map</h2>
             </div>
             <AdminMap 
               zones={zones} 
                  ...
                  <div className="p-5">
                    <form onSubmit={(e) => { e.preventDefault(); handleUpdateConfig(zone.id, new FormData(e.currentTarget)); }}>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                        <div className="space-y-1"><label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Heat Limit (°C)</label><input name="heat_threshold" type="number" defaultValue={zone.heat_threshold} className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></div>
                        <div className="space-y-1"><label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Rain (mm)</label><input name="rain_threshold_mm" type="number" step="1" defaultValue={zone.rain_threshold_mm} className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></div>
                        <div className="space-y-1"><label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Wind (kph)</label><input name="wind_speed_alert_kph" type="number" defaultValue={zone.wind_speed_alert_kph} className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></div>
                        <div className="space-y-1"><label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Vis. Min (km)</label><input name="visibility_min_km" type="number" step="0.1" defaultValue={zone.visibility_min_km} className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></div>
                        <div className="space-y-1"><label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Congestion (0-1)</label><input name="congestion_threshold" type="number" step="0.05" max="1" defaultValue={zone.congestion_threshold} className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></div>
                        <div className="space-y-1"><label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Crowd Limit</label><input name="crowd_density_limit" type="number" defaultValue={zone.crowd_density_limit} className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></div>
                      </div>
                      <div className="flex justify-end pt-2 border-t border-gray-700/50">
                        <button type="submit" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold py-2 px-4 rounded-lg transition-colors shadow-lg shadow-blue-900/20"><Save className="w-3 h-3" /> Save Calibration</button>
                      </div>
                    </form>
                    <div className="mt-6">
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                        {zone.poles.map(p => (
                          <div key={p.id} className="group relative bg-gray-900 border border-gray-700 rounded p-2 flex justify-between items-center hover:border-gray-500 transition-colors">
                             <div className="flex flex-col"><span className="text-xs font-mono text-blue-300">{p.id}</span><span className={`text-[10px] font-bold ${p.status === 'ONLINE' ? 'text-green-500' : 'text-red-500'}`}>{p.status}</span></div>
                             <button onClick={() => handleDeletePole(p.id)} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-500/20 rounded text-red-500 transition-all" title="Decommission Asset"><Trash2 className="w-3 h-3" /></button>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="sticky top-6">
                <div className="bg-gray-800 rounded-xl border border-gray-700 shadow-lg p-6">
                  <div className="flex items-center gap-2 mb-6">
                    <div className="p-2 bg-green-500/10 rounded-lg"><PlusCircle className="w-5 h-5 text-green-400" /></div>
                    <div><h2 className="text-lg font-bold text-white">Deploy Node</h2><p className="text-xs text-gray-400">Add new IoT infrastructure</p></div>
                  </div>
                  ...
                  <form onSubmit={handleAddPole} className="space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <input type="text" placeholder="e.g. MUM-AIR-99" value={newPole.id} onChange={(e) => setNewPole({...newPole, id: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:ring-1 focus:ring-green-500 outline-none" required />
                    </div>
                    ...
                    <button type="submit" className="w-full mt-4 bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-lg transition-colors shadow-lg shadow-green-900/20 flex justify-center items-center gap-2">
                      <PlusCircle className="w-4 h-4" /> Provision Node
                    </button>
                  </form>
                </div>
              </div>
