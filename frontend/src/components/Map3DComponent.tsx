  ...
  useEffect(() => {
    ...
    viewRef.current?.on("click", (event: any) => {
      ...
      viewRef.current?.hitTest(event).then((response: any) => {
        const result = response.results.find((r: any) => r.layer === zoneLayerRef.current);
    ...
    <div className="relative h-full w-full">
      ...
      <div className="absolute top-4 right-4 z-50 bg-gray-900/90 border border-gray-700 p-1 rounded-lg shadow-xl backdrop-blur-sm flex items-center space-x-1">
        <button
          ...
          className={`flex items-center space-x-2 px-3 py-2 rounded-md transition-all text-sm font-medium ${!is3D ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-gray-800'
            }`}
        >
